...
export function TopComplexList({
  complexes,
  selected,
  onSelect,
}: {
  ...
  return (
    <ul className="divide-y divide-neutral-100 dark:divide-neutral-800">
      {complexes.map((complex, i) => {
        const isSelected = selected === complex.complexName;
        return (
          <li key={complex.complexName}>
            <button
              type="button"
              onClick={() => onSelect?.(complex.complexName)}
              disabled={!onSelect}
              className={`flex w-full items-center gap-3 py-3 text-left transition-colors ${
                onSelect ? "cursor-pointer" : ""
              } ${isSelected ? "-mx-2 rounded-lg bg-neutral-100 px-2 dark:bg-neutral-800" : ""}`}
            >
